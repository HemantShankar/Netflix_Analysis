...
-- Deleting empty imdb scores
DELETE FROM netflix.titles
WHERE imdb_score= "";
...
SELECT DISTINCT full_name as actor, 
...
SELECT DISTINCT full_name as director, 
COUNT(*) AS number_of_appearences 
FROM netflix.credits
...
SELECT c.full_name AS actor_actress, 
ROUND(AVG(t.imdb_score),2) AS average_imdb_score
FROM netflix.credits AS c
JOIN netflix.titles AS t 
ON c.id = t.id
WHERE c.roles = 'actor' OR c.roles = 'actress'
AND c.characters = 'leading role'
GROUP BY c.full_name
